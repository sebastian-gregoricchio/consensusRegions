...
reason is the size of the example rather than any weakness in the method.
With only a few hundred peaks per replicate the finest relative rank
available is about 1/430, which is not a small number, so no peak can
accumulate much evidence however well it ranks. On a real peak set of
10^5 calls the ranks are three orders of magnitude finer and the rank
