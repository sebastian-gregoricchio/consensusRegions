...
incomplete input, since not everyone keeps the BAM files or the p-value
column.
...
- \[readPeakSets()\]:
  ...
  reads narrowPeak, broadPeak, BED6 or BED3 files and decides which
  statistics are usable.

- \[computeReplicateWeights()\]:

  derives weights from BAM files, from pre-computed FRiP values, or from
  the peak sets alone.

- \[buildConsensus()\]:

  runs the analysis and returns a \[ConsensusRegions-class\] object.

- \[calibrateThreshold()\]:

  estimates the combined threshold from shuffled peak sets.
...
- Report bugs at
  <https://github.com/sebastian-gregoricchio/consensusRegions/issues>
